...
public final class CoreBlockSkillsRuntime implements AutoCloseable {
    ...
    public CoreBlockSkillsRuntime(PlexonCoreAPI core,Supplier<RuntimeSettings> settings,SkillProgressionService progression,SkillsDiagnostics diagnostics){this.core=core;this.settings=settings;this.progression=progression;this.diagnostics=diagnostics;}

    public void rebuild(){
        closeSubscription();
        RuntimeSettings runtime=settings.get();
        if(runtime.skills().subscribedMaterials().isEmpty())return;
        CoreBlockSubscription spec=CoreBlockSubscription.builder().materials(runtime.skills().subscribedMaterials()).requiresNaturalOrigin(true).build();
    ...
    private void handle(CoreBlockBreakContext context){
        ...
        if(definition.naturalOriginRequired() && context.origin()!=BlockOrigin.NATURAL){
            diagnostics.originRejected();return;
        }
        if(definition.naturalOriginRequired() && runtime.rejectUnknownOrigin() && context.origin()==BlockOrigin.UNKNOWN){
            diagnostics.originRejected();return;
        }
        Player player=Bukkit.getPlayer(context.playerId());
        if(player==null||!player.isOnline())return;
        if(route.skill()==SkillType.FARMING && definition.requireMatureCrop() && !isMature(context)){
            diagnostics.xpRejected();return;
        }
    ...
    private boolean isMature(CoreBlockBreakContext context){
        World world=Bukkit.getWorld(context.worldId()); if(world==null)return false;
    ...
    private void closeSubscription(){if(subscription!=null){try{subscription.close();}catch(Exception ignored){}subscription=null;}}
